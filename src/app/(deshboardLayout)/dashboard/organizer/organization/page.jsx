...
const OrganizationPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const toastId = toast.loading(
      myOrg ? "Updating organization..." : "Creating organization...",
    );

    try {
      let imageUrl = myOrg?.logo;
      if (data.organizationLogo && data.organizationLogo.length > 0) {
      ...
      if (!myOrg) {
        ...
        const updateData = await updateOrganization(newData, myOrg._id);

        if (updateData?.modifiedCount > 0) {
          toast.success("Profile updated successfully", { id: toastId });

          setMyOrg({ ...myOrg, ...newData });
        } else {
  ...
  const logoRegister = register("organizationLogo", {
    required: myOrg ? false : "Logo is required",
